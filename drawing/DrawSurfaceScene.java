...
//		this.stepDistance = 5.5f;
		...
		//this.getCanvas().addGestureListener(ZoomProcessor.class, new DefaultDragAction());
		
	 getCanvas().registerInputProcessor(new ZoomProcessor(getMtApp()));
	 getCanvas().addGestureListener(ZoomProcessor.class, new INF133ZoomAction());
		
	getCanvas().registerInputProcessor(new TapProcessor(getMtApp(), 25, true, 350));
	getCanvas().addGestureListener(TapProcessor.class, new INF133ZoomAction());
...
//								gl.glBlendFuncSeparate(GL.GL_SRC_ALPHA, GL.GL_ONE_MINUS_SRC_ALPHA, GL.GL_ONE, GL.GL_ONE_MINUS_SRC_ALPHA);
								...
								for (int i = 0; i < stepsToTake; i++) { //start i at 1? no, we add first step at 0 already
									...
									getMtApp().pushMatrix();
									getMtApp().translate(diff.x, diff.y);

									//NOTE: works only if brush upper left at 0,0
									getMtApp().translate(brushWidthHalf, brushHeightHalf);
									getMtApp().scale(brushScale);
...
//									mtApp.rotateZ(PApplet.radians(Tools3D.getRandom(-85, 85)));
									getMtApp().rotateZ(PApplet.radians(ToolsMath.getRandom(-25, 25)));
//									mtApp.rotateZ(PApplet.radians(Tools3D.getRandom(-9, 9)));
									...
									//Draw brush
									brushToDraw.drawComponent(getMtApp().g);
	...
	public AbstractMTApplication getMtApp() {
		return mtApp;
	}
