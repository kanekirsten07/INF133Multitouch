...
public class MainDrawingScene extends AbstractScene {
	private static AbstractMTApplication pa;
	private MTRectangle textureBrush; 
	private MTEllipse pencilBrush;
	static public DrawSurfaceScene drawingScene;
	static MTSlider slider = new MTSlider(pa, 0, 0, 200, 38, 0.05f, 2.0f);
	...
	public MainDrawingScene(AbstractMTApplication mtApplication, String name) {
		...
		IFont font = FontManager.getInstance().createFont(pa, "arial.ttf", 35, white);
		//Create window frame
		...
		//		backgroundZoom.setPositionGlobal(new Vector3D(pa.width/2f, pa.height - backgroundZoom.getHeightXY(TransformSpace.GLOBAL),0));
		//	
	...
	public static void setSlider (Float scale ) {
		
		double check = scale.doubleValue();
